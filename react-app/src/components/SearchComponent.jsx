...
const SearchComponent = () => {
  ...
  if(!search){
    ...
    results = users.filter(( data) => data.login.toLowerCase().includes(search.toLowerCase()))
  }

  //fetch users search with their followers
  const fetchUser = async () => {
    try {
      const searchResponse = await fetch("https://api.github.com/search/users?q=YOUR_NAME")
  ...
  return (
    <div>
        ...
        {results.map((user) => (
          ...
          >
            <a href={`/user/${user.login}`}>
              ...
              <div className='flex justify-center mt-5'>
                <img src={user.avatar_url} className='w-6 h-6 rounded-full' alt={`Avatar for ${user.login}`} />
              </div>
            </a>
          </div>
        ))}
      ...
      <div className='flex justify-center align-middle text-center'>
          <h2 className=' capitalize text-4xl font-bold mt-10'>bar chart of user followers</h2>
      </div>
